                                ...
                                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown">
                                    @foreach (Auth::user()->notifications as $notification)
                                        @if ($notification->type == 'App\Notifications\PostNotification')
                                            <a class="dropdown-item border border-0 border-bottom rounded-0 alert alert-primary" href="{{ route('posts.show', $notification->data['post']['id']) }}" role="alert">
                                                New comment on your post: <span class="bg-info-subtle"> {{ Str::limit($notification->data['post']['description'], 50) }}</span>
                                                <br>
                                                Comment: <span class="bg-info-subtle">
                                                    {{ $notification->data['comment']['original_text'] }}
                                                    </span>
                                                <br>
                                    ...
                                    <div class="mx-3">
                                        <form action="{{ route('clear.notification') }}" method="POST">
                                            @csrf
                                            @method('DELETE')
